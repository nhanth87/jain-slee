...
| 4 | `@InjectRa(name="grpcMenuRa")` but RA registered with name `grpc-menu-ra` | **Since ADR 0004 (2026-08-24): boot FAILS FAST** with `IllegalStateException` listing every mismatched field + the registered RA names (escape hatch `-Djainslee.inject-ra.validation=warn\|off`). If no RA is registered at all yet (S5 late-registration flow), the problem is reported loudly at injection time instead — the field stays null and a one-time ERROR names `Class#field → raName`. | Name in `@InjectRa` = value of `RaEndpointPort.getRaName()` exactly character-by-character |
