#!/usr/bin/env bash
# Run example-quarkus-helloworld-web after package.
...
find_app_jar() {
  ...
  if [[ -f target/example-quarkus-helloworld-web-1.0.0-SNAPSHOT-runner.jar ]]; then
    echo "target/example-quarkus-helloworld-web-1.0.0-SNAPSHOT-runner.jar"
    return 0
  fi
  local found
...
if [[ -z "${JAR}" ]]; then
  if [[ -f target/example-quarkus-helloworld-web-1.0.0-SNAPSHOT.jar ]]; then
