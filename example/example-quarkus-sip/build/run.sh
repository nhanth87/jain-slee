#!/usr/bin/env bash
# Run example-quarkus-sip after package.
set -euo pipefail
...
find_app_jar() {
  ...
  if [[ -f target/example-quarkus-sip-1.0.0-SNAPSHOT-runner.jar ]]; then
    echo "target/example-quarkus-sip-1.0.0-SNAPSHOT-runner.jar"
    return 0
  fi
  local found
