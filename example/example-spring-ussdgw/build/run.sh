#!/usr/bin/env bash
# Run example-spring-ussdgw after package.
...
ARTIFACT_ID="example-spring-ussdgw-ussdgw"
...
if [[ -z "${JAR}" ]]; then
  ...
  if ! command -v mvn >/dev/null 2>&1; then
    ...
    echo "  mvn -B -ntp package -Dmaven.test.skip=true" >&2
    exit 1
  fi
...
echo "Starting ${JAR}"
echo "  HTTP RA http://localhost:8081/  (ussd.demo.http.port)"
echo "  health   http://localhost:8081/health"
echo "  gRPC RA  ${USSD_DEMO_GRPC_HOST:-127.0.0.1}:${USSD_DEMO_GRPC_PORT:-9090}"
echo
