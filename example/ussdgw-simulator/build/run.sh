#!/usr/bin/env bash
# Run ussdgw-simulator after package.
# Usage: ./build/run.sh <baseUrl> <msisdn> <ussdString>
set -euo pipefail
...
if [[ ! -f "${JAR}" ]]; then
  echo "note: shaded jar missing — running build.sh first..."
  "${SCRIPT_DIR}/build.sh"
fi

if [[ ! -f "${JAR}" ]]; then
  echo "error: expected ${JAR}" >&2
  exit 1
fi

if [[ $# -lt 3 ]]; then
  echo "usage: $0 <baseUrl> <msisdn> <ussdString>" >&2
  echo "  e.g. $0 http://127.0.0.1:8082 251911000001 '*123#'" >&2
  exit 1
fi
