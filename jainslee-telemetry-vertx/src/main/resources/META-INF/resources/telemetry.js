...
(function() {
  ...
  // ── Alarm clear (global) ──
  window.clearAlarm = function(id) {
    fetch(API_BASE + '/alarms/' + encodeURIComponent(id) + '/clear', { method: 'POST' })
      .then(function(r) {
        if (r.ok) fetchSnapshot();
      }).catch(function(e) { console.warn('clearAlarm failed:', e); });
  };

        '<span class="alarm-source">' + esc(a.source || '—') + '</span>' +
        '<span class="alarm-msg">' + esc(a.message || '') + '</span>' +
        '<span class="alarm-time">' + (a.timestamp ? timeAgo(a.timestamp) : '—') + '</span>' +
        '<button class="alarm-clear" onclick="clearAlarm(\'' + escAttr(a.id || '') + '\')">CLEAR</button>' +
      '</div>';
    }).join('');
  }
      ...
      updateToggleLabel('reconfig-status', snap.autoReconfigEnabled, 'ENABLED', 'DISABLED');
    }
    updateUptime(snap.uptimeSeconds || 0);
  }
