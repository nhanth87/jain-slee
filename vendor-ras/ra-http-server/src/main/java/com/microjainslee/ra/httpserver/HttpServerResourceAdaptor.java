...
/**
 * HTTP server Resource Adaptor — JDK {@link HttpServer} front-end that fires
...
public final class HttpServerResourceAdaptor extends AbstractResourceAdaptor {
    ...
    public void raConfigure() {
        LOG.info(() -> "HTTP server RA configured on port " + port);
    }
    ...
    public void raActive() {
        ...
        ThreadFactory tf = Thread.ofVirtual().name("http-server-", n.getAndIncrement()).factory();
        ...
        LOG.info(() -> "HTTP server RA listening on http://127.0.0.1:" + port);
    }
